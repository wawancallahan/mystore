...
public class Transaksi extends ConnectionDB implements ModelInterface {
    ...
    public List<TransaksiLib> getItemsFilter(String filterBy, String search) {
        try {
    ...
    public ResultSet listFilter(String filterBy, String search) {
        ...
        String query = "SELECT * FROM " + this.table + 
                        ...
                        " FROM transaction_details " + 
                        ") transaction_details ON transaction_details.transaction_id = transactions.id " +
                        " WHERE " + filterBy + " LIKE '%" + search + "%'  ORDER BY transactions.id DESC";
        try {
            ResultSet _ResultSet = super.ExecuteQuery(query);
    ...
    public boolean delete(Integer id) {        
        String query = " DELETE FROM transaction_details WHERE transaction_id = '" + id + "'";
        String query2 = " DELETE FROM transactions WHERE id = '" + id + "'";
        try {
            Integer _ResultSet = super.ExecuteUpdate(query);
            Integer _ResultSet2 = super.ExecuteUpdate(query2);
            
            return _ResultSet2 >= 1;
        } catch (Exception e) {
            System.out.println(e.getMessage());
        }
